...
import { useDeleteAdminCourse } from '../../courses/hooks/useCourseFlow';
  ...
  return (
    <div className="bg-white rounded-xl shadow-md overflow-hidden flex flex-col">
      ...
      <div className="p-4 flex flex-col gap-2">
        ...
        <p className="text-sm text-gray-500">{typeof course.instructor === 'string' ? course.instructor : course.instructor?.name}</p>
        <p className="text-sm text-gray-500">${course.price}</p>
        <div className="flex items-center justify-between gap-2 mt-auto">
          <ButtonUI className="bg-transparent px-6 text-primary-400 border border-primary-400" onClick={handleEdit}>Manage</ButtonUI>
          <ButtonUI className="bg-transparent px-6 text-red-500 border border-red-500" onClick={handleDelete} disabled={deleteCourse.isPending}>Reject</ButtonUI>
        </div>
      </div>
    </div>
  );
